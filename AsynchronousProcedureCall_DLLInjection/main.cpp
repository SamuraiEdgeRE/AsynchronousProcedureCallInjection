...
// 使用方法：[インジェクター名] [ターゲットプロセス名] [DLLのパス]
...
int wmain(int argc, wchar_t* argv[]) {
	...
	if (processHandle == NULL) {
		std::cerr << "[-] OpenProcessが失敗しました。エラーコード: " << GetLastError() << std::endl;
		return 1;
	}
	...
	if (dllPathMemory == NULL) {
		std::cerr << "[-] VirtualAllocExが失敗しました。エラーコード: " << GetLastError() << std::endl;
	...
	if (!successWriteMemory) {
		std::cerr << "[-] WriteProcessMemoryが失敗しました。エラーコード: " << GetLastError() << std::endl;
	...
	BOOL successApc = InjectUsingQueueUserAPC(processId, LoadLibraryW, dllPathMemory);
	if (!successApc) {
		std::cerr << "[-] UsingQueueUserAPCが失敗しました。エラーコード: " << GetLastError() << std::endl;
		VirtualFreeEx(processHandle, dllPathMemory, 0, MEM_RELEASE);
		CloseHandle(processHandle);
		return 1;
	}
